    ...
    return (
        <form action={formAction} className="mt-6 space-y-5">
            <div className="space-y-2">
                <label htmlFor="name" className="block text-sm font-medium text-fg">Project Name</label>
                <input
                    id="name"
                    name="name"
                    type="text"
...
export function ArchiveProjectButton({projectId, projectName, checkedOutQuantity}: ArchiveProjectButtonProps) {
    ...
    return (
        <form
            ...
            onSubmit={(event) => {
                const message = checkedOutQuantity > 0 ? 
        ...
        >
            <button type="submit" className="rounded-md border border-accent/50 px-3 py-1.5 text-sm text-accent transition-colors hover:border-accent hover:bg-accent/10">Archive</button>
        </form>
    );
}