...
export default async function Navbar() {
    ...
    return (
        <header className="h-16 w-full border-b border-border bg-bg/95 backdrop-blur-sm flex items-center justify-between px-6 sticky top-0 z-50">
            <div className="flex items-center gap-6"> 
                <Link href="/" className="flex items-center gap-2.5 group">
                    ...
                    <span className="text-lg tracking-tight font-semibold text-fg font-dmsans">MinoManager</span>
                </Link>

                <nav className="flex items-center gap-10 pl-3">
                    {navItems.map((link) => (
                        <Link key={link.href} href={link.href} className="relative py-2 text-base font-medium text-fg-muted transition-colors hover:text-fg group font-dmsans">
                            {link.label}
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex h-8 items-center gap-2 rounded-full border border-border px-3 py-1 font-medium text-fg">
                    ...
                    <span className="max-w-37.5 truncate text-sm">{session == null ? 'Unknown' : displayName(session.user.firstName, session.user.lastName)}</span>
                </div>

                <form action={logout}>
                    <button type="submit" aria-label="Logout" className="relative p-2 text-fg-muted transition-colors hover:text-fg group">
                        <LogOut size={20} />
                        <span className="absolute bottom-0 left-1/2 h-0.5 w-6 -translate-x-1/2 scale-x-0 bg-destructive transition-transform duration-250 ease-out group-hover:scale-x-100" />
                    </button>
                </form>
            </div>
        </header>
